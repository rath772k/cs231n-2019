...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        softmax = scores - np.max(scores , axis = 1).reshape(N,1)
        ...
        d_score = softmax
        grads['W2'] = H1.T.dot(d_score)/N + 2 *reg * W2
        grads['b2'] = np.sum(softmax, axis = 0).flatten()/N
        H1 = (H1>0).astype('int')
        grads['W1'] = X.T.dot(softmax.dot(W2.T) * H1)/N + 2 * reg * W1
        grads['b1'] = np.sum(softmax.dot(W2.T) * H1, axis =0).flatten()/N
